...
class VoterModelAgent(Agent):
    def __init__(self, aid: int, softmax_prob: float):
    ...
    def prepare_step(self, step_num: int, nbrs: List[Agent], variable_mab: VariableMultiArmedBandit) -> None:
        if step_num == 0:
            ...
            if np.random.uniform() < self.softmax_prob:
                exp_payoff = np.exp(self.payoff_history)
                softmax_probs = exp_payoff / np.sum(exp_payoff)
                self.next_action = np.random.choice(self.action_history, p=softmax_probs)
            else:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {'aid': self.aid, 'atype': self.atype.value, 'softmax_prob': self.softmax_prob}
