...
def get_ns_multi_armed_bandit(mab_start_rounds: List[int]) -> exp.NonStationaryMultiArmedBandit:
    ...
    ns_mab = exp.NonStationaryMultiArmedBandit(multi_armed_bandits=[multi_armed_bandit_1, multi_armed_bandit_2], mab_start_steps=mab_start_rounds)
    return ns_mab


def get_agents(n_agents: int) -> List[exp.Agent]:
    return exp.get_agents(n_agents=n_agents, agent_type=exp.AgentType.VOTER_MODEL, softmax_prob=0.1, memory_decay=0.9, initial_action=3)
...
def run_test():
    ...
    ns_mab = get_ns_multi_armed_bandit(mab_start_rounds=[0, 20])
    networkx = get_network(n_nodes=n_ag, avg_degree=4, rewiring_prob=0.1)
    experiment = exp.run_experiment(eid=0, n_steps=n_steps, agents=agents, non_stationary_multi_armed_bandit=ns_mab, network=networkx)
